...
/*if(fs.existsSync(path.resolve(__dirname, '../.key/mariadb.json'))) {
	data = fs.readFileSync(path.resolve(__dirname, '../.key/mariadb.json'));
	if(data) {
...
const PROJECT = process.env.PROJECT || ''; // 프로젝트 지정(특정 프로젝트) 빌드 - 웹팩 등 빌드 도구(config 파일)내 별도 코드 필요
...
const PORT = (() => {
	return {
		...
		[PHASE_TEST]: 3010,
		[PHASE_STAGE]: 3020,
		[PHASE_PRODUCTION]: 3030,
	}[ACTIVE];
})();
const PORT_SSL = 3100; // https

// window.location.host: 포트포함, window.location.hostname: 포트제외
const PUBLIC_URL = process.env.PUBLIC_URL || 'ecom-proto.cjoshopping.com'; // http://10.88.10.180/
const SOCKET_PORT = process.env.SOCKET_PORT || 3200; // ws://10.88.10.180
const SOCKET_PORT_SSL = process.env.SOCKET_PORT_SSL || 3300; // wss://10.88.10.180
...
const MONGODB_DB = process.env.MONGODB_DB || 'newbie';
const MARIADB_HOST = mariadb.host || '';
const MARIADB_USER = mariadb.user || '';
const MARIADB_PASSWORD = mariadb.password || '';
...
module.exports = {
	...
	socketPortSSL: SOCKET_PORT_SSL,
	mongoPort: MONGODB_PORT,
	mongoHost: MONGODB_HOST,
	mongoDB: MONGODB_DB,
	mariaHost: MARIADB_HOST,
	mariaUser: MARIADB_USER,
